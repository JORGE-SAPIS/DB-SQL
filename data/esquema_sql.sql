...
CREATE TABLE IF NOT EXISTS `celuloide`.`inventory` (
  ...
  CONSTRAINT `fk_inventory_rental1`
    ...
    REFERENCES `celuloide`.`rental` (`inventory_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
...
-- -----------------------------------------------------
-- Table `celuloide`.`film_has_actor`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `celuloide`.`film_has_actor` (
  ...
  INDEX `fk_film_has_actor_film1_idx` (`film_film_id` ASC) VISIBLE,
